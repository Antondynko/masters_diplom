...
def Leader(x, c, V):
    ...
    rho = [[0 for j in range(m)] for i in range(n)]

    c_d = [[0 for j in range(m)] for i in range(n)]
    ...
    for i in range(n):
        for j in range(m):
            if c_d[i][j] != 0 and c_d[i][j] < c_d_min_val:
                c_d_min_val = c_d[i][j]

    for r_idx, row in enumerate(c_d):
        if c_d_min_val in row:
            c_idx = row.index(c_d_min_val)
            min_idx = (r_idx, c_idx)
            break

    rho[min_idx[0]][min_idx[1]] = V - f
